...
def save_project(name, contractor, consultant, location,
    ...
    with _LOCK:
        c = _conn()
        cur = c.cursor()
        cur.execute("SELECT id, subcontractor FROM projects ORDER BY id LIMIT 1")
        row = cur.fetchone()
        if row:
            sub = subcontractor if subcontractor is not None else (row["subcontractor"] or "")
            cur.execute("""
                UPDATE projects
            ...
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (name, contractor, subcontractor or "", consultant, location,
